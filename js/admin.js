document.addEventListener("DOMContentLoaded", () => {
    ...
    // Listen for cross-tab/localStorage changes so admin updates when registration occurs elsewhere
    window.addEventListener('storage', (e) => {
        // Only respond to keys the app uses
        if (!e.key) return;
        if (e.key === 'expo_attendees' || e.key === 'expo_activities' || e.key === 'expo_settings') {
            const isAuth = sessionStorage.getItem("admin_session_auth") === "true";
            // Refresh UI only when admin is unlocked
            if (isAuth) {
                if (e.key === 'expo_settings') {
                    Storage.translatePage();
                    loadSettings();
                }
                updateMetrics();
